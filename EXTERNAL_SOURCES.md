...
камере на Raspberry Pi. Прежде чем продолжать писать `lane_detection_node.py`
с нуля, имеет смысл оценить, не закрывает ли DonkeyCar большую часть задачи
"из коробки" (у него есть готовый веб-интерфейс для сбора данных, обучение
behavioral-cloning моделей вождения, поддержка серво+ESC через PCA9685).
