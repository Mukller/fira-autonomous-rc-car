...
This is intentionally simple (linear falloff) — a real speed profile will
need retuning once lap-time and slip behaviour are measured on the physical
track, and the FIRA scoring gives no credit for finishing off-track.
"""
...
class SpeedControlNode(Node):
    ...
    def on_steering_angle(self, msg: Float32) -> None:
        deflection = abs(msg.data - self.center_angle_deg)
        deflection_ratio = min(1.0, deflection / self.max_deflection_deg)
        throttle = (
            self.max_throttle
            - deflection_ratio * (self.max_throttle - self.min_throttle)
        )
